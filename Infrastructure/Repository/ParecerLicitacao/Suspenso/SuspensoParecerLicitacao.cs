...
{
    public class SuspensoParecerLicitacao : ISuspensoParecerLicitacao
    {
        ...
        )
        {
            ...
            var editais = await context.Editais
                ...
                .Include(x => x.Diretor)
                .Include(x => x.Portal)
                .Where(x => x.Ativo && context.ParecerLicitacoes.Any(p => p.Edital.Id == x.Id && p.Resultado == "suspenso")
                    ...
                    && (empresaId != null ? context.ParecerDiretorComerciais.Any(p => p.Edital.Id == x.Id && p.Empresa.Id == empresaId) : true)
                )
