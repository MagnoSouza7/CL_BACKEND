...
namespace Infrastructure.Repository.Edital.GetTotalEditalGanhos
{
    public class GetTotalEditalGanhos : IGetTotalEditalGanhos
    {
        ...
        )
        {
            ...
            var editais = await context.Editais
                ...
                .Include(x => x.Anexo)
                .AsNoTracking()
                .Where(x => x.Ativo && context.ParecerLicitacoes.Any(p => p.Ativo && p.Edital.Id == x.Id && p.Resultado == "ganhou")
            ...
            foreach (var edital in editais)
            {
                ...
                resposta.Add(new
                {
                    ...
                    NossoValor = await context.ParecerLicitacoes
                        .AsNoTracking()
                        .Where(x => x.Edital.Id == edital.Id)
                        .Select(x => x.NossoValor)
                        .FirstOrDefaultAsync()
                });
            }
