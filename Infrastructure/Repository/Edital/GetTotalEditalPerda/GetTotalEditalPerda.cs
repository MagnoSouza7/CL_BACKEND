...
{
    public class GetTotalEditalPerda : IGetTotalEditalPerda
    {
            ...
            int? empresaId)
        {
            ...
            var editais = await context.Editais
                ...
                .AsNoTracking()
                .Where(x => x.Ativo && context.ParecerLicitacoes.Any(p => p.Ativo && p.Edital.Id == x.Id && p.Resultado == "perdeu")
            ...
            foreach (var edital in editais)
            {
                ...
                resposta.Add(new
                {
                    ...
                    ParecerDiretorEmpresa = await context.ParecerDiretorComerciais
                        .Include(x => x.Empresa)
                        .Include(x => x.Edital)
                        .AsNoTracking()
